...
bool handle_IR(uint32_t delay_time) {
    ...
    if (irrecv.decode(&IR_result)) {
      ...
      int32_t colors[] = {
        0x0000FF, 0xFF0000, 0x00FF00, 0xFFFFFF
      };
 
      int b = (IR_result.value >> (8*2)) & 0xff;
      demo_color = colors[b-1];
      Serial.print(demo_color, HEX); Serial.print(" "); Serial.println((int)b, HEX); 
      Serial.println((long)IR_result.value, HEX);
